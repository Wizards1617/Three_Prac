...
import Supply
cnxn = pyodbc.connect('Driver={SQL Server};Server=DESKTOP-JPDTCD7\MYSERVER;Database=Restoran;Trusted_Connection=yes;')
cursor = cnxn.cursor()

def Admin(adminId):
    _ = system('cls')
    for row in cursor.execute(f"select * from [User] where [ID_User] = '{adminId}'"):
...
def AdminUsersHistory(adminId):
    ...
    for i in range(len(userId)):
        print(f"{userId[i]} - {emailUser[i]} - {passwordUser[i]} - {balanceUser[i]}")
...
def AdminUsersLoyality(adminId):
    _ = system('cls')
    userId, emailUser, passwordUser, balanceUser = [], [], [], []
    for row in cursor.execute("select * from [User]"):
        userId.append(row.ID_User)
        emailUser.append(row.Email_User)
        passwordUser.append(row.Password_User)
